...
async function setupGame() {
    await db.runTransaction(async (transaction) => {
        ...
        if (!gameDoc.exists) {
            ...
            let grid = [];
            for (let i = 0; i < 16; i++) {
                grid.push(i === targetIndex ? targetEmoji : emojis[Math.floor(Math.random() * emojis.length)]);
            }
    ...
    gameRef.onSnapshot(doc => {
        ...
        renderGrid(data.grid, data.targetIndex, data.targetEmoji);
        if (!clicked) {
...
function renderGrid(gridArr, targetIndex, targetEmoji) {
    const grid = document.getElementById('grid');
    grid.innerHTML = '';
    for (let i = 0; i < 16; i++) {
        const btn = document.createElement('button');
        btn.className = 'grid-btn emoji' + (i === targetIndex ? ' target' : '');
